    ...
    return (
        <div ref={gridRef} className="h-full w-full overflow-hidden">
            <section
                ...
                <div className="gap-2 md:gap-4 flex-none relative w-[200vw] md:w-[150vw] h-[200vh] md:h-[150vh] grid grid-rows-4 grid-cols-1 rotate-[-15deg] origin-center z-[2] scale-[0.6] md:scale-100">
                    {[...Array(4)].map((_, rowIndex) => (
                        <div
                            key={rowIndex}
                            className="grid gap-2 md:gap-4 grid-cols-2 md:grid-cols-7"
                            style={{ willChange: "transform, filter" }}
                            ref={(el) => (rowRefs.current[rowIndex] = el)}>
                            {[...Array(6)].map((_, itemIndex) => {
                                const content =
                                    combinedItems[rowIndex * 6 + itemIndex];
                                return (
                                    <div key={itemIndex} className="relative">
                                        <div className="relative w-full h-full min-h-[80px] md:min-h-[120px] overflow-hidden rounded-[10px] bg-[#111] flex items-center justify-center text-white text-[1rem] md:text-[1.5rem]">
